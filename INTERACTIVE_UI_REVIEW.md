...
## Companion card cancel and save — 2026-09-23

Against fc8c784, opened the isolated Korean Collection fixture, selected Mochi and invoked Card save. Native NSSavePanel suggested EvoBar-Mochi. Cancel returned to the same detail, stage2/218XP and same journal; no success message appeared. Invoked again, selected a dedicated mktemp folder using the native Go to Folder sheet, and saved. The UI reported success and retained the same visible record. Decoded and inspected the actual840x1240 PNG: named animal, form, nature, together-days and birth/evolution memories present; token totals, provider split and busiest day absent. This verifies the existing user-facing export path, not just a headless render.

Evidence: build/card-save-interactive.log (terminal9569 exit0); synthetic exported image /tmp/EvoBarCardQA.oBS7Il/EvoBar-Mochi.png retained temporarily for review. Closing the fixture cleaned up its own app/state. No real saves/logs used, no actual collection changed, no upload, no physical VoiceOver claim. Installed runtime unchanged; no rebuild/reinstall needed.
...
# Successful lifecycle checkpoint — 2026-09-23

`./Scripts/review-interactive.sh ko lifecycle` creates a real disposable saved companion at cat6, ready for the final evolution. Unlike presentation scenarios, it supports actual successful writes. Native interaction verified final evolve → graduation sheet → choose dog/name Biscuit → submit → new stage1 companion → Collection → old cat details. Old name,4000XP, seven revealed forms and dated final/graduation journal entries remained visible; only Biscuit was current. Startup/build and39 relevant persistence/pacing tests passed; the graduation regression now reopens its synthetic save and checks record equality. These checks do not prove random-hatch/adoption UI, all-species motion or six-language layout acceptance. Production records were untouched.
# Six-language card/layout checkpoint — 2026-09-23

At4e1ea17, `review-ui.sh en ko ja es fr pt` passed all six isolated runs without retry (build/six-language-review.log). Directly inspected graduated cat card in every locale:24-character synthetic name, dates, birth/latest milestones/final/graduation and footer visible. Also inspected JA/ES/FR/PT compact-ready-long-name: evolution action and progress legible, no clipping. This is sampled layout acceptance, not manual review of every generated image or native-speaker copy certification.12 localization/motion-profile tests passed (build/locale-motion-regression.log); motion-profile checks are not physical animation evidence. No production change or reinstall needed.
# Flight/biped and ground-contact checkpoint — 2026-09-23

Existing interactive tool accepts `motion-flight` (Shiny pterosaur8) and `motion-biped` (Shiny raptor4). Both use presentation-only fixtures and zero pending XP; do not use their IDs for successful save acceptance. Native sampled screenshots show changing wings/legs and scrolling scenery. Raptor initially floated above the ground: transparent square-frame padding caused the mismatch. Production scene now offsets authored walking/biped loops using one cached lowest-visible-pixel baseline across all phases, preserving the artist's relative motion. Flying height and source images are unchanged. Reopened raptor verifies feet meet the ground across sampled poses.

15 authored-animation/stride/motion tests, EN/KO native rendering, Universal2 build/resources/signature/smoke and local installation passed; exact logs/backup in DEVELOPMENT_HANDOFF. No all-variant animation or frame-time claim. Attempted fixture-only Reduce Motion environment override is unsupported because the value is read-only; that failed code was removed. Existing profile tests and static ceremony rendering remain the evidence, not actual system-level Reduce Motion playback. macOS settings were not changed.
