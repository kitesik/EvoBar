...
screen="${2:-collection}"
case "$screen" in collection|collection-duplicates|collection-locked|first-session|motion|motion-flight|motion-biped|lifecycle|hatch-adopt|onboarding|graduation|compact-hatch-error|compact-placement-error) ;; *) echo "Unsupported review screen: $screen" >&2; exit 1 ;; esac
