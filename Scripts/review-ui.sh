...
for locale in "$@"; do
    ...
    for screen in home-incubator collection-incubator hatch-discovery home-egg-warming home-egg-held home-egg-warming-failed home-egg-held-failed hatch-reduced-motion evolution-reduced-motion compact-ready-long-name compact-first-long-name first-session-expanded home-details-expanded switch-failed-detail switch-failed-collection hatch-failed-home hatch-failed-full-home hatch-failed-prompt first-session-growth-egg; do
        image_path="$output_dir/$locale/$screen-dark.png"
        test -s "$image_path"
        sips -g pixelWidth -g pixelHeight "$image_path" >/dev/null
    done
    for screen in shop-expanded settings-tracking-expanded hatch-field-note companion-card-graduated collection-ownership collection-owned-unhatched-detail shop-locked-line-tile record-long-name record-long-korean-name final-record-home compact-final-record-home final-home-scene-theme; do
