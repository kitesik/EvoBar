...
- Shop now leads with ordinary egg and treat; other existing items/scenery are initially collapsed, and the duplicated large wallet is removed. Exact balance remains accessible. No items, prices, balances, entitlements, transaction semantics, growth thresholds or histories changed. Advanced wildcard paths/token-band tuning are collapsed in Tracking settings; connection health/toggles/refresh/errors stay visible. Home's animal scene now exposes the same guarded pet action to accessibility activation.
- Rendering revealed four missing manifest-driven item-name keys. Added all four names in six locales; added a test that every economy-manifest item has a nonempty name in every locale. Native synthetic review checks shop partition/order/full inventory and renders both collapsed and expanded Shop/Tracking states. Current art and payments were not changed.
- Final scoped tests:14 passed in0.242s, covering localization, purchases and saved-game pacing (build/companion-focus-final-tests.log). Final EN/KO native checks and renders passed (build/companion-focus-final-ui.log); inspected Korean/English Shop and expanded Tracking. Universal2 build, resources/signature checks and isolated launch passed (build/companion-focus-final-build.log), terminal1642 exit0. This is scoped local verification, NOT a rerun of the earlier178-test full suite, remote CI, or physical keyboard/VoiceOver acceptance.
- deploy-local terminal75469 exit0, build/companion-focus-deploy.log: /Applications/EvoBar.app rebuilt/installed/launched. Save backup EvoBar-v1.json.before-install-20260923-141556; previous app preserved in TMPDIR/EvoBar-replaced/EvoBar-20260923-141556.app. No live save/log contents read. Unrelated untracked art and Drive mirror preserved. No public release, payment activation, reset credits or new background automation.
- Next bounded review: genuine isolated keyboard/assistive-technology activation of the disclosures/pet control (static renders do not establish that). Remaining art stays Pterosaur4–8; do not call it complete or restart finished lines. Continue simplicity, not feature expansion. App Store remains on hold; Stripe requires separate test-mode/fulfillment scope.

## Seven-line catalog verified and installed — 2026-09-23

- Final local unit suite:178 tests passed in572.259s with EVOBAR_REQUIRE_COMPLETE_ARTWORK=1 (build/seven-line-verify-final.log). That combined run stopped at an intermittent hidden-window Collection search-focus check; unchanged EN/KO UI retry then passed completely (build/seven-line-ui-retry.log). Inspected fresh Korean Collection (7 lines/51 forms) and English Shop images. Do not claim a first-attempt full-pipeline pass or physical keyboard/VoiceOver acceptance.
- Universal2 package, all102 motion strips/408 state PNGs, absence of retired packaged PNGs, signature/ZIP/checksum checks and isolated app launch passed (build/seven-line-package.log; terminal27966 exit0). Remote CI was not run; no notarization/public release/live payments.
- deploy-local terminal22802 exit0: rebuilt/smoke-tested and launched /Applications/EvoBar.app, version0.1.1. Save backup EvoBar-v1.json.before-install-20260923-140438; previous app retained in TMPDIR/EvoBar-replaced/EvoBar-20260923-140438.app. No live save contents read or used as fixtures. Deployment log build/seven-line-deploy.log. Existing launch-at-login registration may need the user to toggle it off/on after replacement.

- New owner direction supersedes App Store preparation: hold App Store publication and consider direct distribution with Stripe. No Stripe integration, live checkout, account/credential changes, or public release is authorized by that consideration request. Existing mock/disabled payment behavior is unchanged.
- Dragon/Phoenix/Kirin removed from animal/lore manifests, individual products, Myth bundle and all-animals grants. Hatching and active UI derive from the retained seven-line catalog; retired instances remain in persistence/export but are excluded from collection targets. Startup now invokes catalog reconciliation before loading UI in both debug and release; missing original starter fails visibly rather than silently resetting data. Invalid pins are cleared. Existing active animals, XP, archived companions and raw entitlement records are preserved by the retirement helper.
- Moved 198 tracked retired sprite PNGs out of package resources into Artwork/RetiredRuntime/2026-09-23 (recoverable, no source-art deletion). Tests/manifest/smoke/render/packaging expectations now cover 7 lines,51 forms,102 variants and58 lore entries. Release gate rejects accidentally packaged retired PNGs. This inventory is NOT a claim of completed V2 art:92/102 variants complete; Pterosaur4–8 remain.
- Targeted first run:21 tests,20 passed; stale lore entry-count expectation failed (83→58), corrected. Synthetic retirement history/relaunch/pin tests passed. First full run exposed stale gait inventory counts (42→35 and84→70); stopped that test process and corrected counts without weakening geometry/alpha checks. Full strict suite restarted in build/seven-line-verify-final.log. SwiftPM retains deleted .process resources in its cache (198 retired PNGs verified still present); build-app now removes only those retired PNGs from the freshly assembled app before signing, and verify-release asserts absence. Record final outcome before committing/installing. Other editors' changes and unrelated untracked art/Drive mirror untouched. Remote CI not run.
- Next after verified local install: finish/review Pterosaur4–8 normal/Shiny art and states; then scope Stripe test-mode/license fulfillment separately. Do not revive the removed mythical lines or the superseded144-variant goal.

## Catalog retirement safety implemented — 2026-09-23

- Added EvoBarStore.reconcileCatalogRetirement, reusing existing ownership reconciliation only when current definition is absent from supplied catalog. Original starter grant required; unavailable current record is retained, original starter resumed or recreated if graduated, entitlements untouched. Rollback restores in-memory state if persistence fails. Available current companions are a no-op.
- Two synthetic tests pass (16469 TERMINAL exit0, build/catalog-retirement-tests.log): removed current history retained with only isCurrent changed, original starter restored, onboarding/entitlement fields unchanged, temporary-file reload and repeated migration idempotent; existing available companion unchanged. Compilation succeeded and diff check clean. No live user save accessed, no installation/full suite yet.
- This is the safety prerequisite, NOT completed species removal: wire into AppModel before first snapshot for both DEBUG/release, remove Dragon/Phoenix/Kirin catalog/storefront/lore entries and retired UI fixture scenarios, update7/51/102 coverage expectations and test active/pinned/migration failure cases. Next complete remaining Pterosaur4–8 and Mac App Store readiness per owner scope above. Ponytail reused existing reconciliation instead of introducing a new migration framework. Preserve other editor files and old allowance note.

## Owner scope supersedes 144-variant goal — 2026-09-23

- Owner explicitly removes Dragon, Phoenix and Kirin (speech transcription "길이" interpreted as Kirin), retains Pterosaur, and requests preparation up to Mac App Store submission, not public release. Old 10-line/144-variant objective is superseded, NOT completed. New catalog target:7 lines,51 forms,102 normal/Shiny variants. Of these92 already have verified V2 art; only Pterosaur4–8 (10 variants) need new art. Runtime catalog removal has NOT yet been implemented. Installedc0f24d6 remains unchanged; latest verified source432cc78.
- Preserve existing instance/XP/history and raw entitlement records without reading live saves. Remove retired species from active catalog, Shop products/Myth bundle, all-animals grants, hatch pool, pin choices and collection targets. Audit current-companion reconciliation in both DEBUG and release: current reconcileReleaseEntitlements skips DEBUG, so simply removing definitions would leave a missing current animal. Reuse EvoBarStore.reconcileVerifiedOwnership where appropriate, with synthetic persistence/relaunch tests. Do not silently reset onboarding or delete historical individuals. Remove obsolete runtime assets only after reference audit; original source art stays preserved.
- Update manifest validation/AppDelegate smoke expectations (10 to7), art/animation gates (72 to51 forms,144 to102 variants), lore/product fixtures, native rendering scenarios and release verification together. Do not weaken tests to hide missing assets. Remaining final-stage fantasy names in retained lines were not requested for removal; do not silently redesign those.
- Ponytail found at https://github.com/DietrichGebert/ponytail, installed using system skill-installer to local skills/ponytail and read fully. Apply native/stdlib/reuse-first minimal implementation, retaining all requested tests, accessibility and data safety. No hooks, account changes or additional plugins installed. Skill available next turn; no background service was started.
- Mac App Store readiness is new work, not equivalent to current RELEASE.md (direct Developer ID distribution). Audit/build sandboxed target, explicit read-only user-selected log-folder grants with persistent security-scoped bookmarks and revoked-access UX; container-safe storage/import preserving existing data; remove external updater/checkout from store build; use StoreKit2 if selling digital animal lines, with mock isolated from release; native sandbox launch-at-login checks. Prepare privacy declarations/required-reason API audit, product metadata/localized screenshots/review instructions and signed archive validation. Apple reference checked2026-09-23: https://developer.apple.com/app-store/review/guidelines/ . Existing unsigned/ad-hoc app is NOT submission-ready. Account/team/product IDs and signing availability remain to be inspected without reading secrets. Do not upload, submit, publish, enroll/pay or change credentials without explicit authorization.
- Live usage now0% weekly used, secondary unavailable; prior quota blocker no longer applies. Next concrete work: implement seven-line removal with synthetic migration coverage, complete Pterosaur4–8, verify/install scoped work, then store-readiness gates. Old allowance-boundary text below is historical. Preserve unrelated untracked artwork and Drive mirror.

## Allowance boundary checkpoint — 2026-09-21

- Previous turn made progress: Pterosaur3 verified and commit432cc78 pushed;2935 terminal0. Current tracked worktree clean, unrelated untracked artwork preserved. No running verification or partial current-stage edits. Ledger98/144, installedc0f24d6 (94 variants); goal incomplete.
- Live weekly allowance10% remaining (90% used), resetsAt1790430616. Secondary unavailable, not zero. Do not start another multi-image/edit/test unit near the guard; recheck live allowance after natural reset before continuing Pterosaur4–8 then Dragon/Phoenix/Kirin. No reset credits or paid usage. Existing thread automation remains paused; no unsolicited restart. Goal remains active; this is a preserved allowance boundary, not completion.
