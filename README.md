...
The current command-line toolchain does not include SwiftData's compiler plugin. Persistence therefore uses a schema-v9, atomic JSON store behind `EvoBarStore`; the app-facing repository boundary is intentionally kept replaceable by a SwiftData adapter once the project is opened with full Xcode.
