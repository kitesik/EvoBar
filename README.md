...
The UI review command renders 52 isolated fixture screens per language in light/dark appearances (English and Korean by default). It includes onboarding, collection detail, item shop, all Settings groups, empty usage, evolution-ready long names, shop results, compact 328-by-374-point panels and sheets, and startup recovery. It also checks startup retry, direct navigation to Tracking settings, and pinning without changing the growing animal. PNGs are saved under `build/ui-review/` and are uploaded by CI as a review artifact. It does not capture the desktop or read real provider logs. These renders supplement, not replace, manual interaction and accessibility checks.
