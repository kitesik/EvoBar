...
The current command-line toolchain does not include SwiftData's compiler plugin. Persistence therefore uses a schema-v7, atomic JSON store behind `EvoBarStore`; the app-facing repository boundary is intentionally kept replaceable by a SwiftData adapter once the project is opened with full Xcode.
...
Next product milestones: supported quota adapters if providers publish suitable APIs, provider status banners, localization, additional providers, production art, real purchase adapter, update channel, and notarized/Homebrew distribution.
