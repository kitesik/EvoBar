...
## Raise a different one without saying goodbye, 2026-09-11

- The product owner found it wrong that reaching the final form was the only way to move on to another companion. It also left the incubator's own hatchlings waiting weeks for a chance. Any individual that has not graduated can now be made the one that grows, from its record in the collection, and the one stepping aside keeps its stage, XP, bond, journal and even the growth it has not taken in yet. Going back to it later finds it exactly where it was left.
- That gives a companion a fourth state and the app a clearer vocabulary: growing now, waiting to be raised (hatched, never raised), resting (raised before, set aside), and graduated (retired for good, and not reversible). Only graduation is final, which is what makes it mean something.
- One thing had to move for this to be safe. The daily gift was keyed to the companion that received the day's first growth, so swapping between two could have drawn it twice. It is keyed to the growth day now. The day's first arrival still counts as care for whichever companion receives it, because that is care for that companion and is capped on its own.

## Feet on the ground, a reveal that waits, 2026-09-22

- Companions looked like they were walking backwards. The authored frame path scrolled the scenery at a fixed 24 points a second for every line and stage, where the procedural rig it replaced had derived that speed from the drawing's own stride. Measured across the bundled strips, the speed the art asks for runs from 1.2 to 24.4 points a second, median 5.4, so the ground was racing past a typical companion four times too fast and the slowest twenty times too fast, dragging its feet backwards. The stride is now measured from the frames once per strip and the ground follows it; a strip that does not walk measures near zero and correctly leaves the ground still.
- The evolution ceremony showed the new form in full colour for the first half second of the alternation, before the white silhouette had covered identity, so the surprise was spent before the white-out. The alternation now waits for the silhouette to be opaque: the shape reads, the identity does not.
- The usage story card said the same thing over every window. It names the window instead: today, the last five hours, this week, this month.

## Turn the cats around, and give the shop something to sell, 2026-09-22

- Five forms walked backwards. The art brief asks for a side profile facing right and every line obeys it except cat stages three to seven, drawn while the V2 style was still being settled; the scene scrolls one way for everyone, so those five faced left while travelling right. Their sprites are mirrored, which is lossless, and their motion strips had their frame order restored afterwards, since flipping a strip also reverses it. The source sheets still face left, so a re-extraction would undo this: that is recorded in the handoff for the run that owns the art.
- One development switch was doing two unrelated jobs, granting every animal line and making every shop item free. Because it was on, the shop showed ten lines all marked owned with no price and no button, and an items tab where nothing cost anything, so coins earned from work bought things that were already free. It is two switches now: the lines stay open, items cost what they say, and the wallet means something.
- With nothing to sell there, the animals tab was a catalogue of lines, which is the collection's job. The shop is the items. Restoring a purchase and importing a licence moved to the foot of it.
- The collection held two different kinds of thing in one grid, a creature the user raised and named beside a species never met, told apart only by a line of small grey print. They are two sections now, with a count each, and every card says in one line what it is and what can be done with it: growing, resting and ready to raise again, graduated with its record kept, or hatch an egg to meet one.

## A grey panel, a switch, and animals that do not talk, 2026-09-22

- The mint accent was the main colour of the whole panel without ever meaning anything, so it competed with the only two things in the app that colour should be saying something about: how rare a companion is, and the companion itself. The chrome is monochrome now, one light on dark glass. Colour is kept for rarity badges, for the sprites and their scenes, for the hearts that measure affection, for the gold of a lucky roll, and for a real warning. The shop's orange wallet and the system blue on the quota bar went with it.
- The provider dot was an orange and a mint. Every place it appears already names the provider beside it, so colour was never what told them apart: two greys now.
- Home's detail area was a chevron and a line of small grey print, which reads as something already open rather than something that can be turned on. It is a switch: pressing it springs the row lit, the label to bold white, and the content in behind it. Released builds up to v0.1.0 had no fold at all, everything on Home at once, which is the state the complaint described.
- The companions were talking in sentences, 59 of them per language, which is a person's way of being present and not an animal's. They make their own noises instead: a cat purrs or hisses, a mammoth pawoos, a kirin chimes. Five sets per line for asleep, at work, sulking, pleased and simply awake, and when it is pleased enough the noise arrives with a heart and throws more. The cries are the animal's, not the reader's, so they are the same in every language and 354 catalog entries went away.
