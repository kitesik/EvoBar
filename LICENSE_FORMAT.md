...
The release build never requires the public key or checkout URL to be committed. `Scripts/configure-storefront.sh` injects `EVOBAR_CHECKOUT_URL` and `EVOBAR_LICENSE_PUBLIC_KEY_BASE64` into the copied app resource bundle before code signing. The protected release workflow requires both values and verifies the HTTPS scheme and decoded 32-byte key length; ordinary DEBUG and unsigned CI builds retain the checked-in `mock-debug` configuration.
