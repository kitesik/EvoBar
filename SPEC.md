...
- The Home tab carries a seven-day usage card between the companion and the evolution progress: one bar per calendar day ending today (quiet days as zero), today solid and earlier days faded, the week's total in the header, and each bar's count on hover. The scene backdrop, the week bars and the progress bar all take their colour from the current sprite's dominant hue rather than the line's manifest colour, so a cosmic tiger never walks in front of an amber sky. Every element on the tab shares one 20 pt horizontal inset.
