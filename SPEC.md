...
- Weekly recap: on the first open of a new calendar week, a card at the top of Home summarises the week that ended (tokens, XP, days worked, the busiest day, and the stages any companion reached in it). It appears only when that week had usage, is closed by hand, and does not return.
...
Daily raw tokens are transformed with diminishing returns: 100% through 1M, 50% from 1M–5M, 20% from 5M–20M, and 5% above 20M. XP is `floor(effectiveTokens / 10,000)`. Daily target XP is recomputed and only the positive difference from already-awarded XP is credited. Credited XP waits, shown only as a translucent run ahead of the fill on the growth bar with the amount beside the count, until the Home tab is on screen; then it arrives on its own: the bar sweeps, the count rolls, and a roll may add to it (15% lucky, half again; 3% golden, doubled plus three Token Coins). A roll never subtracts, and the bonus is a share of what arrived, so opening the panel more often changes nothing. Rare Candy takes the same path and is worth two thirds to four thirds of its listed XP. The first arrival of a growth day also brings a gift, the one draw the calendar paces rather than the work: two to five Token Coins always, a Rare Candy's worth of XP in the same sweep 18% of the time, and a Random Egg 4% of the time. The gift never carries a Mint, because a nature the user can see and hear is not something to reroll unasked.

## Collection loop

The first companion is a directly selected Cat or Dog. One companion grows at a time, but which one can be changed at any moment: any individual that has not graduated can be raised in place of the one growing now, and the one stepping aside keeps its stage, XP, bond, journal and any growth it has not taken in, so going back to it finds it where it was left. That is separate from graduation, which retires a companion that reached its final form. At final evolution, the user may graduate the individual into Collection and either select another owned line or hatch randomly from owned lines. Graduation never erases the individual’s name, dates, usage, XP, provider ratios, nature, rarity, or shiny state. Each individual's record can be saved as a single PNG card: its sprite, name, stage, line, nature, rarity, bond, days together, tokens, XP, provider split, busiest day, and its dates. The file is written only where the user points the save panel; nothing is sent anywhere. Each individual's record carries a journal, written as the moments happen: hatched (shiny or not), first growth, each stage with its date, the busiest day with its tokens, the day affection first reached the top band, the first golden roll, the final form, graduation.

Field guide: every stage, and the real prehistoric relatives met on the way up, is a page in the companion's detail sheet. Sixty-seven pages across the ten lines carry an era, a range, one size figure drawn to scale beside a 1.7 m person, two facts and a field note, the detail an enthusiast would tell a friend. Pages are written in Korean and English and the other four languages read English, so the guide can grow without waiting on six translations of a fact. A page is discovered when any companion of the line, graduated ones included, has reached its stage; a relative opens with the stage it sits after; a locked page shows only its era as a hint. Discovery is derived from the collection and never stored separately. Relatives are guide pages, not playable stages: a stage needs a sprite sheet, a page does not, and lines whose art has not shipped still carry a full guide. The Collection header counts discovered pages beside owned lines, and the lines mastered. A line is mastered when every one of its stages has been reached by some individual, a shiny of it has been raised, and three of its individuals have had different natures; the tile then takes a gold border and a crown, and the line's detail lists the three parts and which are done. Mastery is derived from the collection and never stored.

A Random Egg can instead be placed in the incubator, where it warms on each growth day that saw usage, three of them, and then opens on its own the next time the Home tab is on screen, with the hatch ceremony. What it holds is drawn the same way a graduation hatch is. It becomes a companion waiting to be raised: it is in the collection, it is not growing, and at the next graduation it can be raised in place of choosing a line. Three eggs warm at once. Random hatching draws only from animal lines already owned through the cash storefront. A hatch produces an individual with rarity, one original EvoBar nature, and a normal or shiny variant. Shiny probability is configurable in the economy manifest. Direct selection remains available so randomness never blocks use of a purchased line. Rarity, nature and the shiny mark are shown wherever the individual is (the Home card, the collection tile, the individual's record), with a one-line flavour for the nature, so what a hatch produced can be seen. Every new companion, chosen or hatched, arrives through a hatch ceremony over the Home tab: the egg stirs, cracks and shakes, glows in the line's rarity colour (gold for a shiny), blows out to white, and the companion lands with its name, species and rarity.

Token Coins are earned from daily effective tokens using the same diminishing-return input as XP. Coins cannot be purchased or transferred. The gameplay shop contains Rare Candy for bounded growth, Mint for nature rerolls, Shiny Charm for a configurable shiny-rate modifier, random eggs, and four scene backdrops (dawn, dusk, night, snow) at 60 coins each. A backdrop is bought once, worn at once, and pressed again to take it off so the scene takes its colour from the companion's artwork as before; it changes nothing but how the scene looks. Item effects, prices, and safeguards live in the game-economy manifest.

## Care and bond

- The daily gift belongs to the growth day rather than to a companion, so changing who grows cannot collect it twice; the day's first arrival still counts as care for whichever companion receives it.
...
- Voice: the companion speaks in a bubble over the scene, in its nature's voice. Each of the twelve natures has three idle lines and a reply to petting; sleeping, working and evolution-ready have lines of their own; a sulking or distant mood cuts in half the time; growth has its own lines. It speaks when the panel opens, when petted, when XP arrives, and now and then (every 45 to 90 seconds) while the panel stays open. Lines are catalog strings chosen by `CompanionVoice`, so a Mint changes what the companion says.
- Bond: every act of care a companion ever receives is counted, and that count alone sets a bond level (Newly met, Familiar at 20, Trusted at 60, Close at 150, Inseparable at 300). It never falls, so being away costs nothing already earned, and there is still something ahead once affection has filled. The level's name sits beside the hearts, which keep showing the mood; both are spoken in the tooltip and to VoiceOver, with the care still to give before the next level. Reaching a level is an opt-in notification.
