...
The hatch result now visibly distinguishes a first species discovery from another individual of the same species, using the existing six-language copy. This is presentation only: no species odds, inventory or stored records changed. EN/KO isolated review covers both result states; whether repeats feel worthwhile still needs player feedback.
